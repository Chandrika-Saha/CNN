...
NAME = "Bangla_Compound_Final-{}-conv-{}-dense-{}".format(conv_layers[0],dense_layers[0],int(time.time()))
print(NAME)
model = Sequential()
...
plot_model(model, to_file='CompoundFinal.png')
...
f = open("CompoundFinalConf.csv",'w')
a = []
...
#ff = open("CompoundFinalReport.txt",'w',newline ='')
#ff.write(f1)
#ff.close()
#fff = open("CompoundFinalReportottt.txt",'w',newline ='')
#fff.write(conf)
#fff.close()
print("\nDirectory and predict: ",end-start)
print("Predict: ",end-start1)
model.save('compound.h5')
#plt.figure()
